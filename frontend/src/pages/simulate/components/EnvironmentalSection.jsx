...
export default function EnvironmentalSection({ results }) {
  ...
  // Rebuild donut chart data when theme changes - use useMemo to force recreation
  const donutColors = useMemo(() => {
    return theme === 'bright' 
      ? [chartTheme.seriesColors.secondary, chartTheme.seriesColors.accent]
      : [chartTheme.seriesColors.secondary, '#1a4d45'];
  }, [theme, chartTheme]);
  ...
  }], [results.co2_covered_mt, results.total_country_co2_mt, donutColors, chartTheme, theme]);

  // Rebuild donut layout when theme changes - use useMemo to force recreation
  const donutLayout = useMemo(() => ({
    ...
    annotations: [{
      ...
      x: 0.5,
      y: 0.5,
      font: { size: 22, color: chartTheme.seriesColors.secondary },
      showarrow: false
    }]
  }), [chartTheme, coveragePercent, theme]);

  // Rebuild funnel chart data when theme changes - use useMemo to force recreation
  const funnelColors = useMemo(() => {
    return theme === 'bright'
      ? [chartTheme.seriesColors.accent, chartTheme.seriesColors.secondary, chartTheme.seriesColors.success]
      : [chartTheme.seriesColors.accent, chartTheme.seriesColors.secondary, '#00f5c3'];
  }, [theme, chartTheme]);
  ...
  }], [results.total_country_co2_mt, results.co2_covered_mt, results.co2_reduced_mt, funnelColors, chartTheme, theme]);

  // Rebuild funnel layout when theme changes - use useMemo to force recreation
