...
def load_models():
    ...
    try:
        ...
        print("⚠️  Backend will start but predictions may use fallback formulas")
        print(f"⚠️  Expected model files in: {MODELS_DIR}")
        revenue_model = None
        revenue_encoders = None
        success_model = None
        success_encoders = None
    except Exception as e:
        print(f"⚠️  Warning: Error loading ML models: {e}")
        print("⚠️  Backend will start but predictions may use fallback formulas")
        revenue_model = None
        revenue_encoders = None
        success_model = None
...
def predict_revenue(country, policy_type, carbon_price_usd, coverage_percent, year, fossil_fuel_pct, population, gdp):
    ...
    if revenue_model is None or revenue_encoders is None:
...
def predict_success(country, policy_type, coverage_percent, year, fossil_fuel_pct, gdp):
    ...
    if success_model is None or success_encoders is None:
